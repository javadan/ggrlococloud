...
class Robotable(object):
  ...
  def __init__(self,
               ...
               on_rack=False):
    self.num_motors = NUM_LOCO_MOTORS + NUM_GRIPPER_MOTORS
    self.num_legs = self.num_motors
  ...
  def Reset(self, reload_urdf=True, default_motor_angles=None, reset_time=3.0):
    ...
    #was this the bug? not resetting this?
...
#                                       self._motor_direction[leg_id] * 0) #half_pi)
  ...
  def ConvertFromLegModel(self, actions):
    ...
    # for i in range(self.num_motors):
      ...
      # motor_angle[i] = actions[i]*math.pi/2 #(math.pi + forward_backward_component)# + extension_component)
    return motor_angle
